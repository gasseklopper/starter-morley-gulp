...
class Nav {
	constructor() {
		const show = (...elems) =>
			elems.forEach((elem) => elem.classList.add('visible'))
		const hide = (...elems) =>
			elems.forEach((elem) => elem.classList.remove('visible'))
		...
		const bindMenu = (elem) => {
			...
			items.forEach((item) => {
				...
				const inner = menu.querySelector(
					`.header__menu_inner[data-header-menu-id='${id}']`
				)
		...
		const bindDescriptions = (elem) => {
			elem.querySelectorAll('.header__subitem').forEach((item) => {
				...
				const descriptions = inner.querySelectorAll(
					`.header__description`
				)
				const description = inner.querySelector(
					`.header__description[data-header-description-id='${id}']`
				)
